  ...
  <main class="login">
    <section class="login__panel" aria-labelledby="login-title">
      ...
      <form class="login__form" novalidate @submit.prevent="submit">
        ...
        <button class="login__button" type="submit" :disabled="auth.isLoading">
          {{ auth.isLoading ? 'Validando...' : 'Ingresar' }}
        </button>
      </form>

      <button class="login__link" type="button" @click="goTracking">
...
const dashboardByRole = {
  freight_forwarder: 'ForwarderExpedientes',
  admin: 'AdminUsers',
  supervisor: 'SupervisorDashboard',
  cliente: 'ClienteDashboard',
  soporte: 'SoporteIncidentes'
};
...
async function submit() {
  ...
  try {
    ...
    router.push({ name: dashboardByRole[auth.userRole] || 'RoleDashboard' });
  } catch (error) {
